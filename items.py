...
if recursor_config.get('enabled', False):
    ...
    if node.os == 'debian' and node.os_version[0] >= 13:
        pdns_recursor_configs = {
            ...
            'recursor': {
                ...
                'forward_zones_file': '/etc/powerdns/forward-zones.conf',
        ...
        #   public_suffix_list_file: /usr/share/publicsuffix/public_suffix_list.dat
        #   security_poll_suffix: ''
        # # Validation result: incoming.listen: value `[::]' is not an IP or IP:port combination
        ...
        files['/etc/powerdns/recursor.conf'] = {
            'content': '# Autogenerated by bundlewrap\n' + yaml.dump(pdns_recursor_configs),
            'mode': "0600",
            'owner': 'pdns',
            'group': 'pdns',
            'triggers': [
    ...
    if node.os == 'debian' and node.os_version[0] < 13:
        ...
        forward_zones = ['# Autogenerated by bundlewrap', ]
        forward_zones += [f'- zone: {x}\n  forwarders:\n  - 127.0.0.1:5300' for x in sorted(
            node.metadata.get('powerdns', {}).get('backends', {}).get('bind', {}).get('zones', {}).keys())]
